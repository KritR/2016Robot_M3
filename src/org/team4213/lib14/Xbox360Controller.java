...
public class Xbox360Controller extends Joystick {
	...
	/**
	 * Determine the top speed threshold TODO: This needs to be moved to it an
	 * implementation of AIRLFOContorller, not here directly Bumper buttons on
	 * the controller will limit the speed to the CRAWL value Trigger buttons on
	 * the controller will limit the speed to the SPRINT value Otherwise it will
	 * allow the robot a speed up to Normal max.
	 *
	...
	public double getThrottle(double topSpeedNormal, double topSpeedCrawl, double topSpeedSprint) {
		if (getRawButton(8) || getRawButton(7))
			return topSpeedCrawl; // front-bottom triggers
		else if (getRawButton(6) || getRawButton(9))
